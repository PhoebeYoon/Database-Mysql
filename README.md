...
그렇지 않으면 데이터가 변형되거나 변경될때 어떤 사람은 알고있지만 다른 사람은 모르는 경우가 발생할 수도 있고 사람마다 사용하는 언어와 동일한 데이터도 표현하는 방식이 다르니 혼선이 있을 것입니다 여기에 관리해주는 시스템으로써 <b>DBMS(Database Management System) </b>가 등장한 것입니다.   
...
데이터베이스를 종이에 적어서 관리할 수는 없습니다. 그래서 관리해주는 프로그램을 만들었는데 이것을 DBMS라고 부릅니다. 
