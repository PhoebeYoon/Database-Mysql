...
workbench에서 아래의 명령어를 쿼리탭을 하나 열고 실행합니다.  
```
use mysql;
show tables;
select * from user;
...
workbench 프로그램에서 왼쪽메뉴 중 'Administration' 탭을 찾아 선택합니다 왼쪽메뉴 중에서 'Users and Privieges'를 선택합니다.     
...
> (아래의 예제에서 general1 과 general2를 같은 사용자로 취급해주세요. 윈도우와 맥에서 예제를 만들다보니 각각 실행상의 차이가 있어서 저는 2개를 만들어서 사용했지만 여러분은 general1 또는 genearl2를 같은 것으로 취급하시면 됩니다 )    


아까 실행했던 쿼리탭에서 다음을 실행합니다    
```
create user 'general2'@'localhost' identified by '1234';
```   
'Administration' 탭 > 'Administrative Roles'탭 열어보면 
general2이라는 유저를 생성되어 있습니다. 그리고 이 사용자가 사용할 비번으로 '1234', 접속하는 위치는 로컬컴으로 등록했습니다. 
...
아래와 같이 터미널에서 실행해보세요. 우리는 workbench 프로그램에서 'root' 사용자로 접속했을때 보여던 다른 디비가 'general2' 사용자로 접속했을때는 보이지 않는다는 것을 확인할 수 있습니다.  

<img width="485" alt="스크린샷 2023-04-30 오후 8 13 12" src="https://user-images.githubusercontent.com/48478079/235349976-c887ef6a-1fe9-4423-b442-0cbd0754572e.png">




이제 workbench프로그램의 상단왼쪽의 🏠 클릭해서 'MySQL Connections'화면으로 넘어갑니다.  
➕ 클릭하여 'general2' 유저가 사용할 접속을 만들어보겠습니다.   
...
'general2'사용자로 접속해서 왼쪽패널의 'Administration' 탭을 클릭하고 'User Accounts'에서 'general1'를 선택합니다.   
그리고 오른쪽 화면에 'Administrative Roles'를 선택하면 아래와 같이 보일것입니다. (아래 실행화면은 윈도우에서는 다르게  나올수 있습니다)   
