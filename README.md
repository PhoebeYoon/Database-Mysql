...
데이터베이스의 구조와 제약조건에 대한 명세를 기술한것이며 데이터 저장을 나타내는 추상적인 디자인을 말합니다. 아래 이미지에서 테이블에 있는 첫번째 줄이 스키마를 정의한 것이라고 보셔도 좋습니다 
...
우리는 이미 mysql를 설치하면서 윈도우의 명령프롬프트창에서 아래의 명령어를 실행합니다.  
```   mysql -uroot -p    ```   
이제   이것을 해석해보면, mysql를 실행하되 u(유저)는 'root'라는 이름을 사용한다는 의미입니다 -p는 패스워드를 사용하겠다는 의미이고요    
